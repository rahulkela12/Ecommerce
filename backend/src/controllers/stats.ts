  ...
  const allOrderPromise = Order.find({}).select([
    ...
    "shippingCharges"]);
      ...
      const [processingOrder,shippedOrder,deliveredOrder,
        categories,productsCount,productsOutofStock,allOrders,
...
export const getLineCharts = TryCatch(async(req,res,next)=>{
 ...
 const productCount= new Array(12).fill(0);
  ...
  products.forEach((i)=>{
    ...
    const orderMonthDiscount = new Array(12).fill(0);
       const orderMonthRevenue = new Array(12).fill(0);

       orders.forEach((order)=>{
         const creationDate =  order.createdAt;
         const monthDiff = (today.getMonth() - creationDate.getMonth() + 12)%12;
          if(monthDiff < 12){
             orderMonthDiscount[12-monthDiff-1] += order.discount;
             orderMonthRevenue[12-monthDiff-1] += order.total;
          }
        });
